...
class InfoViewController: UIViewController {
    private let howToUseText = "\n\nWebCamMap uses API from api.windy.com to show webcams around the world.\n\nYou can select point on map, and below map is the list of selected locations. If you click on any element of that list, and if there are any webcams within 20km of selected location, app will show it. You can then click on webcam you want and app will open browser and go to api.windy.com to show you that webcam you've selected. There you can see day timelapse of that webcam.\n\nAlso, you can add to favourites locations which webcams you like. You add location to favourites by clicking on movie circle image which is positioned on the right of every element(location) in the list.Favourites are automatically moved to top of the list, and saved in storage. When you open app again, favourites are there.\n\nYou want to remove location from favourites? Click again on movie circle image, and location is removed from favourites and moved below all favourite locations."
